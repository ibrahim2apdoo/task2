...
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <apan class="text-muted">{{__('words.show_news_letters')}}</apan>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
                ...
                <table class="table table-separate table-head-custom table-checkable" id="kt_datatable">
                    ...
                    @foreach($news_letters as $key => $news_letter)
                        <tr>
                            ...
                            <td>{{createdAtFormat($news_letter->created_at)}}</td>
                            <td>{{createdAtFormat($news_letter->created_at) == updatedAtFormat($news_letter->updated_at) ? '--' : updatedAtFormat($news_letter->updated_at)}}</td>
                            <td nowrap="nowrap">
                                @include('admin.components.form-controls', ['name'=>'news-letters', 'value'=>$news_letter,'role'=>'news_letters'])
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
