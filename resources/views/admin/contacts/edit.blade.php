...
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <apan class="text-muted">{{__('words.edit_contact')}}</apan>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
...
@section('form_content')
    ...
    <div class="card card-custom">
        <div class="card-body">
                ...
                <div class="form-group col-6">
                    ...
                    <div class="input-group">
                        ...
                        <select class="form-control" id="type" name="type">
                            ...
                            <option
                                value="phone" {{old('type',$contact->type) == 'phone' ? 'selected' : ''}}>{{__('words.phone')}}</option>
                            <option
                                value="email" {{old('type',$contact->type) == 'email' ? 'selected' : ''}}>{{__('words.email')}}</option>
                            <option
                                value="social" {{old('type',$contact->type) == 'social' ? 'selected' : ''}}>{{__('words.social')}}</option>
            ...
            <div class="form-group row">
                @include('admin.components.icon',['label'=>__('words.icon'),'value'=>old('icon',$contact->icon)])

            </div>

            <div class="form-group row">
                @include('admin.components.switch',['label'=>__('words.status'),'name'=>'status','val' => old('status',$contact->status)])
            </div>
